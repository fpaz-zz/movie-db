...
export const updateImagePathWithBaseURL = (movieResult, width = 300) => ({
    ...movieResult,
    backdrop_path: movieResult.backdrop_path ? `${TMDB_IMAGE_BASE_URL(1280)}${movieResult.backdrop_path}` : null,
    poster_path: movieResult.poster_path ? `${TMDB_IMAGE_BASE_URL(width)}${movieResult.poster_path}`: null,
  });

export const updateMoviesImagePath = ({ results }, width) => results ? results.map(movieResult => updateImagePathWithBaseURL(movieResult, width))
    : null;
