...
class MovieDetails extends Component {
  ...
  componentDidMount() {
    ...
    if (this.props.currentMovie.id !== id) {
      ...
      this.props.getMovieDetails({ movieId: id }).then(() =>
        this.setState({
          isLoading: false
        })
      );
  ...
  render() {
    const {
      ...
      overview,
      vote_average
    } = updateImagePathWithBaseURL(this.props.currentMovie);
    ...
    return isLoading ? (
      ...
      <Container className="movie-details">
        <div className="arrow-back" onClick={this.handleBackClick.bind(this)} />
        <Card className="transition-item">
          <div className="card-banner" style={styles} />
          <Card.Body>
            <Row>
              <Col xs={5} md={3} lg={2}>
                <img className="thumbnail" alt="thumbnail" src={poster_path} />
              </Col>
              <Col xs={7} md={9} lg={10}>
                <div className="card-info">
                  <h5>{title}</h5>
                  <p>
                    {new Date(release_date).getFullYear()} &#183; {popularity}%
                    User Score <br />
                    {timeConvert(runtime)}
                  </p>
                </div>
